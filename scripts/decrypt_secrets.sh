...
if [[ -n "$ENCRYPTED_GOOGLE_SERVICE_PASSWORD" ]]; then
  ...
  openssl enc -aes-256-cbc -pbkdf2 -d -k "$ENCRYPTED_GOOGLE_SERVICE_PASSWORD" -in ./encryptedFiles/GoogleService-Info.plist.enc -out ./ios/GoogleService-Info.plist -md md5
fi

openssl enc -aes-256-cbc -pbkdf2 -k "$ENCRYPTED_GOOGLE_SERVICE_PASSWORD" -out ./encryptedFiles/google-services.json.enc -in ./android/app/google-services.json -md md5